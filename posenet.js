...
function onMotion(poses) {
    ...
    for (let i = 0; i < parts.length; i++) {
        if (parts[i].part === "Brain") {
            if (calDistance(parts[i], rightWrist) < 400) {
                // console.log("rightWrist", parts[i].part, calDistance(parts[i], rightWrist), leftWrist.confidence)
                console.log("rightWrist", parts[i].part)
            }
            if (calDistance(parts[i], leftWrist) < 400) {
                // console.log("leftWrist", parts[i].part, calDistance(parts[i], leftWrist), leftWrist.confidence)
                console.log("leftWrist", parts[i].part)
            }
        }
    }
}
